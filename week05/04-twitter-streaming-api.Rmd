...
```{r}
authentication <- list(consumer_key = "jjzLLmoXLm8GwrRLL7LSVrMsf",
                 consumer_secret = "ucqgtTTUcIRxxvQ7S6NAYCVkgAFakdEXPH3YklqQYUld5Y5uno",
                 access_token = "1452328279428509710-xeYtXHbrGHxnFKd4731n8hd6MRw1dY",
                 access_token_secret = "LBxw1vlxYMwyedrpzy3KTcD22snUmEr3ax0KW4DB0Xu72")
```
...
twitter_token <- create_token(app = "week05/04", 
...
```{r}
stream_tweets(q = "", timeout = 30, file_name='teststreamed_tweets.json')
sample_tweets <- tryCatch({parse_stream('teststreamed_tweets.json')},
                       error = function(e)
                           {print(paste("Retrying with alternative function after initial error when parsing file",
                                        'teststreamed_tweets.json'));
                           return(recover_stream('teststreamed_tweets.json'))})
head(sample_tweets)
...
```{r}
keywords_tweets <- stream_tweets("Obama AND Biden", timeout = 5)

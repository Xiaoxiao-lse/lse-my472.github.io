...
```{r}
uol <- read_file("uol.txt")

#str_view_all(uol,"United \\w+")

postcode <- str_extract_all(uol,"[A-Z]{1,2}[0-9]{1,2}[A-Z]?\\s?[0-9][A-Z]{2}")
...
uol <- str_replace_all(uol,"([A-Z]{1,2}[0-9]{1,2}[A-Z])?\\s?[0-9][A-Z]{2}","\\1")

postcode <- str_extract_all(uol,"[A-Z]{1,2}[0-9]{1,2}[A-Z]?\\s")
...
dfm <- data_char_ukimmig2010 %>%                        # 1. study the data
  corpus() %>% 
  tokens(remove_punc = TRUE,  remove_numbers= TRUE) %>%    # 2. remove punctuation and numbers
  tokens_remove(stopwords("English")) %>%                        # 3. remove stopwords (English)
